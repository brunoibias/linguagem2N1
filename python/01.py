class Number:
    def read_numbers(self):
        ...
        while True:
            new_list.append(
                int(input("Digite a %d° quantidade de Rebeldes de presos " % counter)))
            option = int(input("Digite? \n1 = Para adicionar mais escória Rebelde \n0 = Para ver o resultado \n"))
            counter += 1
...
print("Maior conjunto de Rebeldes pressos : %d" % bigger)

smaller = number.find_little(numbers_new_list)
print("Menor conjunto de Rebeldes pressos: %d" % smaller)

mySum = number.sum_values(numbers_new_list)
print("Soma de Rebeldes pressos: %.2f" % mySum)

average = number.calculate_average(mySum, numbers_new_list)
print("Média do conjunto de Rebeldes pressos: %.2f" % average)

approximate = number.find_approximate(average, numbers_new_list)
print("Valor aproximado da média de Rebeldes pressos: %.2f" % approximate)
